...
namespace TMPro.Examples
{
	...
	public class TMP_TextInfoDebugTool : MonoBehaviour
	{
		...
		private void OnDrawGizmos()
		{
			...
			this.ObjectStats = "Characters: " + this.m_TextInfo.characterCount + "   Words: " + this.m_TextInfo.wordCount + "   Spaces: " + this.m_TextInfo.spaceCount + "   Sprites: " +
							   this.m_TextInfo.spriteCount + "   Links: " + this.m_TextInfo.linkCount + "\nLines: " + this.m_TextInfo.lineCount + "   Pages: " + this.m_TextInfo.pageCount;
		...
		private void DrawCharactersBounds()
		{
			...
			for (int i = 0; i < characterCount; i++)
			{
				...
				bool isCharacterVisible = i < this.m_TextComponent.maxVisibleCharacters && characterInfo.lineNumber < this.m_TextComponent.maxVisibleLines &&
				...
				_ = this.m_Transform.TransformPoint(new Vector3(characterInfo.bottomRight.x, characterInfo.bottomRight.y, 0));
				...
				else
				{
					...
					this.DrawDottedRectangle(this.m_Transform.TransformPoint(new Vector3(characterInfo.origin, characterInfo.descender, 0)),
											 this.m_Transform.TransformPoint(new Vector3(whiteSpaceAdvance, characterInfo.ascender, 0)), color, 4);
				}
		...
		private void DrawWordBounds()
		{
			for (int i = 0; i < this.m_TextInfo.wordCount; i++)
			{
				...
				for (int j = 0; j < wInfo.characterCount; j++)
				{
					...
					bool isCharacterVisible = characterIndex <= this.m_TextComponent.maxVisibleCharacters && currentCharInfo.lineNumber <= this.m_TextComponent.maxVisibleLines &&
											  (this.m_TextComponent.overflowMode != TextOverflowModes.Page || currentCharInfo.pageNumber + 1 == this.m_TextComponent.pageToDisplay);
		...
		private void DrawLineBounds()
		{
			...
			for (int i = 0; i < lineCount; i++)
			{
				...
				bool isLineVisible =
					(lineInfo.characterCount != 1 || firstCharacterInfo.character != 10 && firstCharacterInfo.character != 11 && firstCharacterInfo.character != 0x2028 &&
						firstCharacterInfo.character != 0x2029) && i <= this.m_TextComponent.maxVisibleLines && (this.m_TextComponent.overflowMode != TextOverflowModes.Page ||
